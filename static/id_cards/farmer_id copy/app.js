...
function resetForm() {
  ...
  tableBody.innerHTML = '';
  addLandTableRow('UTTAR PRADESH', 'SIDDHARTH NAGAR', 'Birdpur', '124', '*', '0.150000');
    ...
    div.innerHTML = `
      <div class="queue-card-preview-mini">
        ...
        <span class="disclaimer">This card is strictly for personal use and does not constitute a government-issued identification.</span>
              <span class="footer-hi">फार्मर आईडी</span>
            </div>
          </div>
        </div>
...
/* ==========================================================================
   ...
   ========================================================================== */
function printSingleCardDirectly(card) {
  ...
  // Allow QR code generation to populate DOM elements in the hidden node before opening print dialog
  setTimeout(() => {
...
function printSingleCardDirectlyFromUI(card) {
  printSingleCardDirectly(card);
}

function printSingleCardDirectlyFromEvent() {
  // Gathers data active on Creator Form
  const farmerId = document.getElementById('farmer-id').value;
  const nameEn = document.getElementById('name-en').value || 'Sita Kumari';
  const nameHi = document.getElementById('name-hi').value || 'सीता कुमारी';
  const dob = document.getElementById('dob').value;
  ...
  printSingleCardDirectly(mockCard);
}

function printSingleCardDirectly() {
  // Detect if click comes from Creator page single card preview button
  const farmerId = document.getElementById('farmer-id').value;
  const nameEn = document.getElementById('name-en').value || 'Sita Kumari';
  const nameHi = document.getElementById('name-hi').value || 'सीता कुमारी';
  const dob = document.getElementById('dob').value;
